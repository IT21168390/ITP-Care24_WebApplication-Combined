...
export default function AddPayroll() {
    ...
    useEffect(() => {
        if (params.id) {
            function getPayroll() {
                axios.get(`http://localhost:4500/payroll/add/get/${params.id}`).then((res) => {
            ...
            //     axios.get("http://localhost:4500/payroll/").then((res) => {
    ...
    function sendData(e) {
        ...
        //alert("Hi" + name)
        axios.post("http://localhost:4500/payroll/add", newPayroll)
            .then(() => {
                Swal.fire({
                    icon: "success",
                    title: "Payroll Added!",
